...
def predictSingle(model,data,batchsize):
    model.eval()
    # arraytestdata = np.load(PREPATH)
    # data = torch.from_numpy(arraytestdata)
    temp =  data
...
def predict(model,path,batchsize):
    data = np.load(path)
    result = []
    for i in range(data.shape[0]):
        d =  torch.from_numpy(np.array([data[i]]))
        result.append(predictSingle(model, d, batchsize))
    return result
...
if __name__ == '__main__':
    ...
    MODELPATH = 'C:\\Users\\WU\\Desktop\\NUS\\IOT\\Project\\model.pt'
    ...
    print(predict(model, SINGLETESTPATH, BATCH_SIZE))
